...
| 4. Gateway and enterprise | Least-privilege gateway profiles, quota/route truth, review through restricted company gateways, and external worker routes | [Provider profiles](./PROVIDER_PROFILES.md); [offline enterprise-gateway campaign contract](./ENTERPRISE_GATEWAY_CAMPAIGN.md) proves fail-closed identity/quota/retry/redaction checks on a loopback fixture, including needle-free public errors and refusal to qualify from hand-labeled `live_campaign` fields; live restricted-gateway, quota, Cursor-account (`CURSOR_CLOUD_API_BASE` + run/campaign id), HTTPS retry/idempotency, and release-artifact receipts remain unverified | In progress |
...
| 6. Operator UX | Fast power-user workflows, keyboard/screen-reader correctness, understandable approvals and recovery | The integrated Help Center at `45441d1f` adds focus trapping, Escape/focus restoration, responsive layout, reduced-motion and forced-colors handling; full desktop suite is 47 files / 274 tests; independent expert acceptance remains | In progress |
| 7. Semantic help | Searchable in-app help with contextual guidance and an optional assistant boundary | [`HelpCenter`](../desktop/src/components/HelpCenter.tsx) and [`helpCenter.ts`](../desktop/src/lib/helpCenter.ts) provide 19 stable source-cited articles spanning core operation, recovery, queue steering, MCP, broker embedding, external cloud workers, Computer Use, and providers, with deterministic paraphrase search, explicit offline/provider retrieval labels, confirmation-gated semantic ranking, bounded assistant context and provider-response ceilings, strict answer validation, and helper-session cleanup; live provider/corpus qualification remains | In progress |
| 8. Embeddable platform | Stable Rust DTOs, desktop adapter, browser-safe broker, reusable UI primitives, and external agent adapters | [Cross-product ADR](./ADR-003-cross-product-capability-surface.md), [embedding guide](./EMBEDDING.md), [broker protocol](./WEB_BROKER_PROTOCOL.md), [Cursor Cloud design](./CURSOR_CLOUD_INTEGRATION.md), [SDK](../crates/common/grokptah-agent-sdk/README.md) | Contract, response-validation, headless UI, source-cited Help Center exports (`c7800a46`), hardened browser authority checks (`b17e901f`), installable Tauri-free package staging including the `./ui-core` subpath, an installed `npm pack` consumer smoke check, provider-neutral external-worker DTOs, and a trusted native Cursor Cloud v1 lifecycle adapter are staged; publication, trusted broker routes, live qualification, and cross-repository integration remain |
