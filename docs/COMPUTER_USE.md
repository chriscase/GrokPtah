...
| Isolated visual substrate decision | source candidate | Disposable measured VM; no default network, host clipboard/share, credential forwarding, or host input injection. Typed isolated input, a no-launch OS/framework probe, manifest/lifecycle contracts, authenticated read-only metadata protocol, path-free open-handle content measurement, fixed-path strict app/helper identity verification, and an unshipped least-privilege helper plus credentialed assembler are candidate source only. The helper source accepts only inherited handles/private pipes and configures no network/share/audio/storage/host-input devices. Virtualization authority is reserved for that future signed sandboxed helper, not the main process. No reviewed guest/built helper/assembled app or runtime signing, carrier/rendering, launch/cleanup, or dispatch evidence exists; #288 remains open. |
