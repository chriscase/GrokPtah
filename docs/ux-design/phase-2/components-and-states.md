...
| Agent detail | views.js | Identity + Start-Lane form (runtime radio with live connection chips) + grouped Lanes + D04 Build-only ad-hoc assignment with primary-resume limitation; lifecycle actions marked "Proposed"; retired shows "Proposed: unretire…" | active, paused, retired (banner + blocked form) |
