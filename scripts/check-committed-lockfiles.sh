...
# roots, each with its own committed Cargo.lock. Adding a path dependency to one
# crate silently invalidates the lockfile of every *other* workspace that
...
#     dozens of unrelated packages. Refreshing those here would be an unrelated
#     dependency bump, so it is reported and not gated.
