 ...
 *    coordinates the source bytes are actually digested in. UTF-8 is the only
 *    range a non-JavaScript peer — a host authority, an audit tool, another
 *    language's client — agrees on without re-encoding, so it is the range
 *    coverage and overlap are decided in.
...
export type HelpClaimSpan = {
   ...
   * a JavaScript renderer and a non-JavaScript peer agree on without
   * re-encoding.
   */
