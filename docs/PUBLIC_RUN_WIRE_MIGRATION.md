...
1. **SDK** `grokptah-agent-sdk::dto::{project_run, project_runs, RunView}` — strips secrets client-side from camelCase `RunRecord`; keeps `session_id`, `workspace`, `bounds`, `stop_cause`. Crate must not depend on `grokptah-agent-bridge`; consume the same schema, do not fork a second DTO.
...
4. **Desktop protocol** `desktop/src/lib/protocol.ts` `DurableRun` + `api.ts` `remoteServiceRunList` / `remoteServiceRunGet` (same type as local host `run_list` / `run_get`, which stay internal).
5. **Desktop UI** `App.tsx` remote watch (`sessionId`, `workspace`, `startSeq`); `runOrigin.ts` (`clientId`); `RunInspector.tsx` (`promptPreview`, `aggregates`, `bounds`, `stopCause`, `finalResponse`, `progress.lastTool` / `detail`).
...
3. **Split desktop local vs remote types.** Keep `DurableRun` for in-process host. Remote parser uses `parse_public_run_*` (bridge) / fail-closed TS parse of the same keys. Stamp `sessionId`/`workspace` from the list/get *request*, never from the run document. Watch scopes follow that stamp; `startSeq` → `eventStartSeq`. `RunInspector` remote mode must not read `promptPreview` / path aggregates / `clientId`.
4. **SDK parser** accepts only `grokptah.public-run.v1` + allowlisted keys; unknown version/field → existing redacted `SdkError::Internal`. Map into `RunView` from DTO + request `SessionScope`/`RunSelector`. Drop or optionalize `bounds`/`stop_cause` to match step 1. Bump `CONTRACT_VERSION` if `RunView` shape changes. No duplicate `from_run` conversion.
