...
impl HelperSupervisor {
    pub fn new(world: HelperWorld) -> Self {
        Self::with_ledger(world, None)
    }

    pub fn with_ledger(world: HelperWorld, ledger: Option<PathBuf>) -> Self {
        let used = ledger
            .as_ref()
            .map(|path| load_ledger(path).unwrap_or_default())
            .unwrap_or_default();
        Self {
    ...
    pub fn dispatch(
        ...
        let intent = intent_digest(dispatch_id, lease, action);
        let mut inner = self.inner.lock().expect("helper supervisor");
        if let Some(existing) = inner.used.get(dispatch_id) {
            if existing.intent_digest != intent {
                return Err(ComputerError::new(
                    ComputerErrorCode::Conflict,
                    "dispatch_id was reused with a different action or lease binding",
                ));
            }
...
impl HelperSupervisorInner {
    fn finish_dispatch(&mut self, dispatch_id: &str, mut receipt: EffectReceipt) -> EffectReceipt {
        ...
        persist_ledger(
            ...
            dispatch_id,
            &self.used[&dispatch_id.to_string()],
        );
        receipt.cleanup = self.cleanup(true);
        self.used.insert(dispatch_id.to_string(), receipt.clone());
        persist_ledger(self.ledger.as_deref(), dispatch_id, &receipt);
        receipt
    }
    ...
    fn cleanup(&mut self, release_lease: bool) -> CleanupReceipt {
        ...
        CleanupReceipt {
            ...
            frames_released: self.world.temp_artifacts == 0,
            replay_blocked: self.session.is_none(),
        }
    }
}
...
fn intent_digest(dispatch_id: &str, lease: &HelperLease, action: &ComputerAction) -> String {
    let payload = serde_json::json!({
        ...
        "observationId": lease.observation_id,
        "observationGeneration": lease.observation_generation,
        "target": lease.target,
        "action": action,
    });
...
fn load_ledger(path: &Path) -> ComputerResult<HashMap<String, EffectReceipt>> {
        ...
        .map_err(|error| ComputerError::new(ComputerErrorCode::Internal, error.to_string()))?
    {
        ...
        let raw = fs::read(entry.path())
            .map_err(|error| ComputerError::new(ComputerErrorCode::Internal, error.to_string()))?;
        if let Ok(receipt) = serde_json::from_slice::<EffectReceipt>(&raw) {
            used.insert(receipt.dispatch_id.clone(), receipt);
        }
    }
    Ok(used)
}

fn persist_ledger(ledger: Option<&Path>, dispatch_id: &str, receipt: &EffectReceipt) {
...
#[cfg(test)]
mod tests {
    use super::super::package_identity::SigningClass;
    use super::*;
    ...
    #[test]
    fn duplicate_id_with_changed_action_fails_closed() {
        let (supervisor, lease) = lease_and_supervisor(HelperWorld::granted_demo());
        supervisor
            .dispatch("dispatch-1", &lease, &set_value())
            .unwrap();
        let invoke = ComputerAction::Invoke {
            element_id: "submit".into(),
        };
        assert_eq!(
            supervisor
                .dispatch("dispatch-1", &lease, &invoke)
                .unwrap_err()
                .code,
            ComputerErrorCode::Conflict
        );
    ...
    #[test]
    fn durable_ledger_reloads_after_process_cut() {
        let dir = tempfile::tempdir().unwrap();
        let supervisor = HelperSupervisor::with_ledger(
            HelperWorld::granted_demo(),
        ...
        let lease = supervisor
            ...
            .unwrap();
        supervisor
            .dispatch("dispatch-ledger", &lease, &set_value())
            .unwrap();
        drop(supervisor);
        let reloaded = HelperSupervisor::with_ledger(
            HelperWorld::granted_demo(),
            Some(dir.path().to_path_buf()),
        );
        let invoke = ComputerAction::Invoke {
        ...
        assert_eq!(
            reloaded
                .dispatch("dispatch-ledger", &lease, &invoke)
                .unwrap_err()
                .code,
            ComputerErrorCode::Conflict
        );
