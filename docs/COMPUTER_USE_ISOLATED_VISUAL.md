...
```sh
cd /private/tmp/grokptah-cu-packaged-vm-authority-integration-v1
# After ≥25 GiB free, unoccupied target, cryptographically admitted helper bundle
# and guest.img (+ manifest), not empty helper.signed / guest.img.signed markers:
GROKPTAH_ISOLATED_VISUAL_ARTIFACT_ROOT=/path/to/admitted-artifacts \
  cargo test --locked --manifest-path crates/codegen/grokptah-isolated-visual/Cargo.toml -- --test-threads=1
GROKPTAH_ISOLATED_VISUAL_ARTIFACT_ROOT=/path/to/admitted-artifacts \
  cargo run --locked --manifest-path crates/codegen/grokptah-isolated-visual/Cargo.toml \
  --bin grokptah-isolated-visual-qualify
```

Real VF boot is allowed only after `IsolatedPreflight.allowed_to_launch` is
true **and** an observed launch receipt sets `evidence_class` to
`virtualization_framework`. Eligibility without launch cannot mint that class.
