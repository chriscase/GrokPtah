...
impl IsolatedSimulator {
    ...
    pub fn fail_next_input(&mut self) {
        self.fail_next_input = true;
    }

    pub fn attached(&self, guest_id: &str) -> bool {
        self.frames.contains_key(guest_id) || self.inputs.contains_key(guest_id)
    }
    ...
    pub fn accept_input(&mut self, event: IsolatedInputEvent) -> IsolatedResult<()> {
        if self.fail_next_input {
            ...
            return Err(IsolatedError::internal(
                "injector failed after durable Injected write",
            ));
        }
