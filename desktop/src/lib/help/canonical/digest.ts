/**
 * Dependency-free SHA-256 for canonical corpus and model digests.
 *
 * The public `@grokptah/client` bundle is browser-safe and has no runtime
 * dependencies, and `crypto.subtle` is async-only. Corpus binding has to be
 * synchronous at module load so a stale digest fails closed before any
 * retrieval runs, so the hash is implemented here with the exact FIPS 180-4
 * operations. Only integer arithmetic is used, which is bit-exact on every
 * JavaScript engine.
 */
...
export function sha256Hex(value: string): string {
  ...
  // Lengths beyond 2^32 bits are not reachable for a bounded corpus payload,
  // so the high word stays zero and the low word carries the bit length.
 ...
 *
 * Two structurally equal payloads always serialize to the same bytes, so a
 * digest computed here is stable across engines, key insertion order, and
 * corpus refactors that do not change content.
 */
