...
/// Cases pinning the TypeScript digest implementation to this one.
///
/// A browser bundle cannot call Rust, so the digest rules exist twice. Two
/// implementations that merely intend to agree eventually disagree, so these
/// vectors are emitted from the Rust side and asserted by the TypeScript
/// tests. They deliberately include the cases where a naive implementation
/// diverges: multi-byte characters (length is counted in UTF-8 bytes, not
/// characters), a field containing the separator a non-injective scheme would
/// have used, an empty field, and the same field list under two domains.
#[must_use]
pub fn digest_parity_cases() -> serde_json::Value {
    let cases: Vec<(&str, Vec<&str>)> = vec![
        (
            crate::digest::domain::SOURCE,
            vec!["provider.profiles", "docs/P.md", "Configure"],
        ),
        (
            crate::digest::domain::CHUNK,
            vec!["provider.profiles", "docs/P.md", "Configure"],
        ),
        (crate::digest::domain::ARTICLE, vec![]),
        (crate::digest::domain::ARTICLE, vec![""]),
        (crate::digest::domain::ARTICLE, vec!["", ""]),
        (crate::digest::domain::REQUEST, vec!["a|b"]),
        (crate::digest::domain::REQUEST, vec!["a", "b"]),
        (crate::digest::domain::CLAIM, vec!["\u{1F600}"]),
        (crate::digest::domain::CLAIM, vec!["café", "naïve"]),
        (crate::digest::domain::GRANT, vec!["1:2", "3"]),
        (
            crate::digest::domain::RECEIPT,
            vec!["line\nbreak", "tab\there"],
        ),
        (
            crate::digest::domain::MANIFEST,
    ...
    let emitted: Vec<serde_json::Value> = cases
        ...
        .map(|(domain, fields)| {
            serde_json::json!({
                "domain": domain,
                "fields": fields,
                "lengthPrefixed": crate::digest::length_prefixed(&fields),
                "digest": crate::digest::domain_digest(domain, &fields),
            })
        })
        .collect();

    let hashes: Vec<serde_json::Value> = ["", "abc", "café", "\u{1F600}", "a\nb"]
        .into_iter()
        .map(|input| {
            serde_json::json!({ "input": input, "sha256": crate::digest::sha256_hex(input) })
        })
        .collect();

    serde_json::json!({
        "note": "Generated by help-codegen. Asserted by the TypeScript digest tests so the \
