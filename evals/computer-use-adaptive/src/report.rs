...
use crate::types::{
    AdapterId, CampaignStatus, Eligibility, FamilyId, REPORT_SCHEMA, SOURCE_GATE_SHA,
};
...
pub fn fixture_hash(items: &[Scenario]) -> String {
    let mut hasher = Sha256::new();
    for s in items {
        hasher.update(s.id.as_bytes());
        hasher.update(s.family.as_str().as_bytes());
        hasher.update(s.seed.to_le_bytes());
        hasher.update(s.objective.as_bytes());
        hasher.update([s.adversarial as u8, s.held_out as u8]);
    }
...
pub fn run_campaign(repeats: u32, seed: u64) -> CampaignOutput {
    let items = catalog();
    validate_catalog(&items).expect("catalog");
        ...
        for profile in &scenario.profiles {
            for adapter in &scenario.adapters {
                for rep in 0..repeats {
                    let _ = seed;
                    let bundle = run_episode(scenario, *profile, *adapter, rep);
    ...
    CampaignOutput { report, evidence }
}
...
pub fn assemble_report(
    ...
    CampaignReport {
        ...
        naming: NamingRecord::decision_packet(),
        repeats,
        seed,
        provider_calls,
        status,
        ...
        families: present,
        metrics,
        fixture_hash: fixture_hash(items),
