...
| No permit forgery or model-send bypass | A send permit has private fields; `begin_send` is its only producer. Bridge `provider_transport` consumes a fresh permit before its single `Client::execute`; host chat, agent-step, provider-qualification, and OIDC token refresh traffic all route through it, and a static source guard pins those call sites. It is not the sole credential-bearing model wire: sampler `provider_admission::send_admitted` and the memory embedding path also perform `OperatorSendHost` admission before one execute. Possible-write failures remain Uncertain and are not auto-resent; only proven NotSent connect failures may start a new admitted attempt. |
