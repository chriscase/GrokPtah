...
impl AuditLedger {
    pub fn retain(&self, request: RetentionRequest) -> AuditResult<RetentionReceipt> {
        ...
        if !descriptor.origin_authenticated {
            return Err(AuditError::Refused(RefuseReason::GenerationUnverified));
        }
        ...
        if let Some(seal_id) = request.export_seal_id.as_deref() {
            if !self.has_valid_export_seal(seal_id, &descriptor.generation_id)? {
                return Err(AuditError::Refused(RefuseReason::ExportSealInvalid));
            }
        }
        ...
        self.append_transaction_record(
            AuditEntryInput::new(
                "audit.retention",
                EntryPhase::Intent,
