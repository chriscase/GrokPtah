...
async fn request_timeout_returns_error() {
    ...
    let resp = reqwest::Client::new()
        ...
        .await
        .unwrap();
    // Timeout maps to HTTP 504 + stable public category `internal` with
    // bounded `reasonCode=timeout`.
    assert_eq!(
        resp.status(),
...
async fn unknown_and_forbidden_tools_fail_closed_over_http() {
    ...
    for name in [
        ...
        let body: serde_json::Value = resp.json().await.unwrap();
        // json_err projects the internal diagnostic onto the stable public
        // forbidden_scope envelope.
        assert_eq!(
