...
- Optional `browser-engine` feature is receipt-gated: no receipt → no frame bytes; live macOS WK rasters mint a process-private receipt then complete; public engine evidence, admission, and Computer Mode stay fail-closed. Ordinary `cargo test` never latches the physical-CLI authorize flag. Drafts #563 (clipboard kill-gate) and #564 (snapshot ABI / `takeSnapshot` block) are not absorbed and still need rebase onto current main.
