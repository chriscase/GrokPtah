...
- Isolated service `cargo test --locked -- --test-threads=1` on a fresh `GROKPTAH_HOME`, checked out at SHA `8cecd488ebb3f5b34692e70f09bb89cc2c8ec3ad` (tree `2ade073c8ac6496c46d428a1b16993bc6d33b079`): exit 0 (`SERVICE_TEST:0`). `disconnect_reconnect_restart_and_cursor_expiry_are_durable` passed in that run.
- Isolated service `cargo check --locked --all-targets` on that same checkout: exit 0 (`SERVICE_CHECK:0`).
...
| Gap | Result on `8cecd488` / `2ade073c`. Prior functional SHA/tree: `d7306c1e7d346d1ee9ed6f36e06e457d5a65c6c6` / `a121de35fe498bc9fb147f53ccfa5f034949d95f`. | Named regression |
| --- | --- | --- |
...
| R5 validation | Checked out at `8cecd488` / `2ade073c`: host-authority `AUTHORITY:0`, desktop typecheck `TC:0`, npm test `NPM:0` (58 files, 428 tests), bridge fmt `FMT:0`, Clippy `CLIPPY:0`, bridge suite `SUITE:0`, isolated service test `SERVICE_TEST:0`, service check `SERVICE_CHECK:0`, Tauri `TAURI:0` (50 lib tests). Desktop run `36083488303` `headSha` is exactly `8cecd488ebb3f5b34692e70f09bb89cc2c8ec3ad`. | `foreign_workspace_receipt_collision_is_ignored_and_unchanged`; `idempotency_is_principal_scoped_and_scope_bound` |
...
Hosted Desktop for repaired functional SHA `8cecd488ebb3f5b34692e70f09bb89cc2c8ec3ad` only: GitHub Actions run `36083488303` attempt 1 (https://github.com/chriscase/GrokPtah/actions/runs/36083488303) concluded `success`. The event was `pull_request`. `gh run view 36083488303 --json headSha` reports `headSha` `8cecd488ebb3f5b34692e70f09bb89cc2c8ec3ad`, which is exactly that functional SHA. The `desktop` job had no failed steps (`2026-09-25T01:46:55Z` to `2026-09-25T02:13:58Z`).
