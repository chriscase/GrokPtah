...
| Isolated visual substrate decision | source candidate | Disposable measured VM; no default network, host clipboard/share, credential forwarding, or host input injection. Typed isolated input, a no-launch OS/framework probe, manifest/lifecycle contracts, authenticated read-only metadata protocol, and path-free open-handle content measurement are candidate source only. Virtualization authority is reserved for the future signed helper, not the main process. Helper/guest packaging, code-signing/entitlement proof, carrier/rendering, launch/cleanup evidence, and dispatch are not implemented; #288 remains open. |
