...
export type HelpAbstentionReason = "no-query" | "below-threshold" | "empty-corpus";
...
function indexFor(corpus: HelpCorpus): Index {
  const cached = INDEX_CACHE.get(corpus.digest);
  if (cached) return cached;
  const built = buildIndex(corpus);
  INDEX_CACHE.set(corpus.digest, built);
  return built;
}
...
export function searchHelpCorpus(
  ...
  const best = scored[0];
  if (!best || best.score.fused < HELP_ABSTENTION_THRESHOLD) {
