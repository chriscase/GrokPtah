...
///
/// Readiness is a property of this process, not of the caller, so this path
/// needs capacity without an identity. It gets there through the narrow
/// [`ReadinessAuthority`] (#477) rather than a fabricated `AuthContext`: an
/// unauthenticated local probe must not hold a value indistinguishable from a
/// real caller's identity, and since the fence closed, one cannot be built at
/// all. The authority grants exactly this one read and nothing else.
///
/// This returns a bare `bool` by construction: the diagnostic payload is
/// dropped inside this function and cannot reach an unauthenticated response.
fn unauthenticated_readiness_verdict(state: &AppState) -> bool {
    let payload = state
        .orch
        .capacity_for_readiness(&ReadinessAuthority::internal());
    payload_is_ready(&payload)
}
