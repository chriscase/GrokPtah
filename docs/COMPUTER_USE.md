...
| Isolated visual substrate decision | source candidate | Disposable measured VM; no default network, host clipboard/share, credential forwarding, or host input injection. Typed isolated input, a no-launch OS/framework probe, manifest/lifecycle contracts, authenticated read-only metadata protocol, path-free open-handle content measurement, and fixed-path strict app/helper identity plus entitlement verification are candidate source only. Virtualization authority is reserved for the future signed sandboxed helper, not the main process. No helper/guest/configuration or signing pipeline is packaged, and there is no runtime signing proof, carrier/rendering, launch/cleanup evidence, or dispatch; #288 remains open. |
