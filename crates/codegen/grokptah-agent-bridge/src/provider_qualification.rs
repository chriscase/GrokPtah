...
/// Install the qualification-send envelope during an explicit host policy
/// transition. The qualification request body is intentionally absent from
/// the snapshot: arbitrary probe payloads may select the envelope's operation,
/// but cannot mint or widen its authority.
pub(crate) fn preinstall_qualification_envelope(
    provider_id: &str,
    model_id: &str,
    ...
    let credentials =
        crate::auth_store::resolve_wire_credentials_for_model(&crate::gateway_config::model_selection_key(
            &provider_id,
            model_id,
        ))
        .map_err(anyhow::Error::msg)?
    ...
    let provider_model = profile
        ...
        .ok_or_else(|| anyhow!("model `{model_id}` is not registered on provider `{provider_id}`"))?;
    let wire_model_id = provider_model.wire_model_id();
    ...
    authority.install_canonical_envelope(
        &format!("qualification-send:{}:{model_id}", principal.id()),
        snapshot,
        principal.id(),
