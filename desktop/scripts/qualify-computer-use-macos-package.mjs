...
function sha256File(path) {
  ...
  if (stat.isSymbolicLink()) {
    throw new Error(`refusing to hash symlink: ${path}`);
  }
  if (!stat.isFile()) {
    throw new Error(`refusing to hash non-file path: ${path}`);
  }
...
function hashBundleManifest(root) {
  ...
  function walk(current, relative) {
    ...
    if (!stat.isFile()) {
      throw new Error(`bundle member is not a file: ${relative}`);
    }
...
function inspectApp(appPath) {
  ...
  const codesign = codesignDisplay(app);
  const identifierMatch = /Identifier=(\S+)/.exec(codesign);
  const teamMatch = /TeamIdentifier=(\S+)/.exec(codesign);
  const helperPath = join(
    app,
    "Contents/Helpers/GrokPtah Computer Use Helper.app",
  );
  let helperCodesign = null;
  let helperManifest = null;
  if (existsSync(helperPath)) {
    helperCodesign = codesignDisplay(helperPath);
    helperManifest = hashBundleManifest(helperPath);
  }
  return {
    present: true,
    ...
    appManifestSha256: hashBundleManifest(app),
    helperManifestSha256: helperManifest,
  };
}
...
function main() {
  ...
  const evidence = {
    ...
    kind: "source_topology_inspector",
    sourceHead: head,
    branch,
    ...
    appManifestSha256: inspected.appManifestSha256 || null,
    helperManifestSha256: inspected.helperManifestSha256 || null,
    teamId: inspected.teamId || null,
