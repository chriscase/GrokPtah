...
describe("Help route", () => {
  ...
  it("states the offline case honestly rather than as an error", () => {
    ...
    expect(note.textContent).toMatch(/No answer provider is configured/);
  ...
  it("names the provider when one is configured, and when it is not reachable", () => {
    const { rerender } = render(
      <HelpRoute open onClose={vi.fn()} provider={{ kind: "configured", providerLabel: "Company gateway" }} />,
    );
    ...
    rerender(
      <HelpRoute
        open
        onClose={vi.fn()}
        provider={{ kind: "unavailable", providerLabel: "Company gateway", detail: "quota exhausted" }}
      />,
    );
