...
| 8. Embeddable platform | Stable Rust DTOs, desktop adapter, browser-safe broker, reusable UI primitives, and external agent adapters | [Cross-product ADR](./ADR-003-cross-product-capability-surface.md), [embedding guide](./EMBEDDING.md), [broker protocol](./WEB_BROKER_PROTOCOL.md), [Cursor Cloud design](./CURSOR_CLOUD_INTEGRATION.md), [SDK](../crates/common/grokptah-agent-sdk/README.md) | Contract, response-validation, headless UI, source-cited Help Center exports (`c7800a46`), hardened browser authority checks (`b17e901f`), installable Tauri-free package staging including the `./ui-core` subpath, an installed `npm pack` consumer smoke check, and provider-neutral external-worker DTOs are verified; publication, cross-repository integration, and Cursor adapter remain |
