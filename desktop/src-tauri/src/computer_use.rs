...
use grokptah_agent_bridge::{
    ...
    AdaptiveAttemptOutcome, AdaptiveProfileProjection, ObservationFingerprint, ProfileTransition,
    RuntimeSignal,
...
impl DesktopComputerUse {
    ...
    pub async fn run_model_turn(
        ...
        let evidence = self
            ...
            .map_err(|error| error.to_string())?;
        let risk = classify_task(objective, &observation);
        let permit = service
            .begin_adaptive_turn(run_id, owner_session_id, &evidence, risk)
            .map_err(|error| error.to_string())?;
        ...
        let outcome = if attempt.outcome.is_ok() {
            AdaptiveAttemptOutcome::Succeeded {
                observation_bytes: attempt.rendered.bytes,
                truncated: attempt.rendered.truncated,
            }
        } else {
            AdaptiveAttemptOutcome::Failed {
                observation_bytes: attempt.rendered.bytes,
        ...
        let raw = match attempt.outcome {
            ...
            Err(error) => {
                ...
                return Err(match signal {
                    Some(signal) => {
                        format!("{detail} {}", self.resolve_adaptive_signal(&service, run_id, signal))
                    }
                    None => detail,
                });
            }
        ...
        let result = self
            .apply_model_proposal(owner_session_id, run_id, expected_version, observation_id, raw)
            .await?;
        if result.completed {
    ...
    pub async fn apply_model_proposal(
        ...
        let accepted = accept_model_proposal(&context, &raw)
            .map_err(|error| self.refuse(&service, run_id, error))?;
        // Profile narrowing runs *after* the universal seal and can only
        // reject more. A run with no adaptive record has no profile to narrow
        // by, which is the operator-driven path.
        if let Some(profile) = run.adaptive.as_ref().map(|record| record.profile) {
    ...
    pub async fn approve_simulator_action(
        ...
        // Capture the one host-issued frame that verifies this dispatch. It
        // both restores a current observation for the next model turn and is
        // the only thing that can turn the run's action receipt into
        // completion evidence, for that frame and no other (#456).
        //
        // A failure here is deliberately not fatal to the approval: the action
        // already happened. The receipt simply stays unverified, so completion
        // remains impossible until a later dispatch earns its own frame.
        let acted = owned_run(&service, owner_session_id, &pending.run_id)?;
...
mod tests {
    ...
    use grokptah_agent_bridge::{
        ActionOutcome, ComputerBackend, ComputerErrorCode, ComputerStore, ComputerTarget,
    };
    ...
    fn raw(value: serde_json::Value) -> RawModelProposal {
    ...
    async fn fabricated_model_completion_cannot_terminate_a_run() {
        ...
        assert!(after
            .audit
            .iter()
            .any(|entry| entry.disposition == "refused"
