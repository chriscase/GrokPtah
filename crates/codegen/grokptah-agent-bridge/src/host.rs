...
use crate::host_helpers::{
    ...
    auto_cargo_reverify_command, bounded_tool_output_with_integrity, build_agent_messages,
    build_compact_summary, call_xai_agent_step_observed, call_xai_chat,
    cargo_test_failure_coaching, cargo_test_output_failed, cargo_test_output_passed,
    cargo_test_reverify_coaching, clip_tool_output_for_wire, coding_agent_tools,
    ...
    is_round_limit_stop_message, multi_failure_partial_edit_coaching, normalize_sandbox_profile,
    observe_accepted_tool_progress, offline_plan_steps, parse_effort_arg,
...
impl AgentHostHandle {
    ...
    pub fn review_run(&self, session_id: Uuid, run_id: &str) -> Result<RunReview> {
        self.with_promotion_lock(run_id, || {
            self.review_run_locked(session_id, run_id, true)
    ...
    pub(crate) fn inspect_run(&self, session_id: Uuid, run_id: &str) -> Result<RunReview> {
        self.with_promotion_lock(run_id, || {
            self.review_run_locked(session_id, run_id, false)
                .map(|outcome| outcome.review)
        })
    }
    ...
    pub fn promote_run_with_approval(
        ...
        self.with_promotion_lock(run_id, || {
            ...
            if durable_approval.is_none() && current_review.diff_truncated {
                bail!(
    ...
    pub fn keep_run_for_review(&self, session_id: Uuid, run_id: &str) -> Result<RunRecord> {
        self.with_promotion_lock(run_id, || {
            ...
            match run.client_id.as_deref() {
                Some("desktop") => {}
                _ => bail!("keep for review is limited to local desktop runs"),
            }
            if run.state != RunState::Completed {
    ...
    async fn run_mcp_tool(
        ...
        match result {
            Ok(out) => {
                let clipped = bounded_tool_output_with_integrity(&out, 24_000);
                let _ = event_tx.send(SessionUpdate::ToolCallUpdate {
                    session_id,
                    call_id,
                    status: ToolCallStatus::Completed,
...
mod tests {
    ...
    fn keep_discard_and_apply_are_terminal_and_mutually_exclusive() {
        ...
        let promoted_id = "keep-after-apply";
        ...
        let discarded_id = "keep-after-discard";
        save_isolated_desktop_run(
            &host,
            session_id,
    ...
    fn keep_run_for_review_collapses_unsafe_cases_without_success() {
        ...
        let cross_session = "keep-cross-session";
        save_isolated_desktop_run(
            &host,
            session_id,
            workspace.path(),
        ...
        let cross_workspace = "keep-cross-workspace";
        save_isolated_desktop_run(
            &host,
            session_id,
            workspace.path(),
        ...
        let unreviewed = "keep-unreviewed";
        save_isolated_desktop_run(
            &host,
            session_id,
            workspace.path(),
