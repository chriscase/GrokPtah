...
use grokptah_isolated_visual::{
    ...
    IsolatedCleanupReason, IsolatedErrorCode, IsolatedGuestTerminal, IsolatedVisualHost,
    IsolatedVisualResourceLimits, IsolatedVisualStore, TestClock,
};
...
fn cleanup_failure_is_uncertain_and_guest_is_not_cleaned() {
    ...
    host.terminate(&guest.guest_id, IsolatedCleanupReason::GuestCrash)
        .unwrap();
    let mut observation = host.observe_cleanup(&guest.guest_id).unwrap();
    if let Some(overlay) = observation.overlay_removed.as_mut() {
        overlay.overlay_present = true;
    }
    assert_eq!(
        host.cleanup_from_observations(&guest.guest_id, observation)
            .unwrap_err()
            .code,
